...
def analyse(para):
    ...
    for sentence in sentence_list:
        ...
        neu = neu + vs['neu']
        pos = pos + vs['pos']
        comp = comp + vs['compound']
    ac = str(round(comp / len(sentence_list), 4))
    aneg = str(round(neg / len(sentence_list), 4))
    apos = str(round(pos / len(sentence_list), 4))
    aneu = str(round(neu / len(sentence_list), 4))

    return aneg, aneu, apos, ac
...
def get_sentiment():
    with open('Quora.csv', 'r', encoding= 'UTF-8') as f:
        ...
        for line in reader:
            print(line[2])
            v1,v2,v3,v4 = analyse(line[2])
            with open("user_compound.csv", 'a', newline='', encoding='utf-8') as csvfile:
                fieldnames = ['UserId', 'compound_ans_value']
                writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
                writer.writerow({'UserId': line[1], 'compound_ans_value': v4})
